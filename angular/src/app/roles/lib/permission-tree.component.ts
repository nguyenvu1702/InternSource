...
export class PermissionTreeComponent extends AppComponentBase implements AfterViewInit {

    @Output() onSelectedChange = new EventEmitter<StringFlatTreeSelectDto[]>();
    permissionNames: StringFlatTreeSelectDto[] = [];
    ...
    getGrantedPermissionNames(): StringFlatTreeSelectDto[] {
        ...
        for (let i = 0; i < selectedPermissions.length; i++) {
            this.permissionNames.push(new StringFlatTreeSelectDto({
    ...
    refreshTree(): void {
        ...
        let treeData = _.map(this._editData.permissions, function (item) {
            return {
                ...
                state: {
                    opened: true,
                    selected: _.includes(self._editData.grantedPermissionNames, item.id)
                }
            };
        });
